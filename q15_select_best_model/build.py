...
def complete_build(x_train, x_test, y_train, y_test):
    cat_index = [x for x in range(len(x_train.columns)) if x_train[x_train.columns[x]].dtype == 'object']
    x_train_t, x_test_t = ohe_encode(x_train, x_test,cat_index)
    ...
    # print (pd.concat([complete_stats1, complete_stats],axis=0))
